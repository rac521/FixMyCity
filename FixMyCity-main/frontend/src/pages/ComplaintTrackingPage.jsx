...
import { ThumbsUp, MapPin, Clock, X, Bell } from 'lucide-react';
...
export default function ComplaintTrackingPage() {
  ...
  useEffect(() => {
    if (complaints.length > 0) {
      ...
      const dismissed = JSON.parse(localStorage.getItem('dismissedResolutions') || '[]');
      
      const newlyResolved = complaints.filter(c => 
        myIssues.includes(c.id) && 
        c.status === 'Resolved' && 
        !dismissed.includes(c.id)
      );
      
      setNotifications(newlyResolved);
    }
  }, [complaints]);
  ...
  const dismissNotification = (id) => {
    const dismissed = JSON.parse(localStorage.getItem('dismissedResolutions') || '[]');
    dismissed.push(id);
    localStorage.setItem('dismissedResolutions', JSON.stringify(dismissed));
  ...
  const handleUpvote = async (id) => {
    ...
    try {
      ...
      // Track upvoted issue locally so dashboard can notify on resolution
      const myIssues = JSON.parse(localStorage.getItem('myIssues') || '[]');
      if (!myIssues.includes(id)) {
        myIssues.push(id);
      ...
      <div className="animate-slide-right delay-300" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'end', marginBottom: '2rem' }}>
        ...
        <div style={{ position: 'relative' }}>
          <button 
            ...
            style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '50%', padding: '0.75rem', cursor: 'pointer', position: 'relative', color: 'var(--text-main)', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}
            onMouseOver={(e) => e.currentTarget.style.background = 'rgba(255,255,255,0.1)'}
            onMouseOut={(e) => e.currentTarget.style.background = 'rgba(255,255,255,0.05)'}
          >
            <Bell size={24} />
            {notifications.length > 0 && (
              <span style={{ position: 'absolute', top: '-5px', right: '-5px', background: 'var(--danger)', color: 'white', fontSize: '0.75rem', fontWeight: 'bold', width: '22px', height: '22px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 10px rgba(239, 68, 68, 0.5)' }}>
          ...
          {showNotifications && (
            <div className="glass-panel animate-fade-in" style={{ position: 'absolute', top: '100%', right: 0, marginTop: '0.75rem', width: '350px', zIndex: 50, padding: 0, overflow: 'hidden' }}>
              <div style={{ padding: '1rem', borderBottom: '1px solid rgba(255,255,255,0.1)', fontWeight: 'bold', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span>Notifications</span>
                {notifications.length > 0 && (
                  <span style={{ fontSize: '0.8rem', background: 'var(--primary)', color: 'white', padding: '2px 8px', borderRadius: '10px' }}>{notifications.length} New</span>
                )}
              </div>
              ...
              <div style={{ maxHeight: '350px', overflowY: 'auto' }}>
                {notifications.length === 0 ? (
                  ...
                  notifications.map(n => (
                    <div key={n.id} style={{ padding: '1rem', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', background: 'rgba(16, 185, 129, 0.05)' }}>
                      <div>
                        <h4 style={{ margin: '0 0 0.25rem 0', color: '#10b981', fontSize: '0.95rem' }}>Issue Resolved</h4>
                        <p style={{ margin: 0, color: 'var(--text-muted)', fontSize: '0.85rem', lineHeight: '1.4' }}>
                          An issue you supported (<strong>{n.subcategory}</strong>) was marked as resolved!
                        </p>
                      </div>
                      <button 
                        onClick={() => dismissNotification(n.id)}
                        style={{ background: 'rgba(255,255,255,0.1)', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', padding: '0.25rem', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                        title="Dismiss"
                      >
                        <X size={16} />
                      </button>
