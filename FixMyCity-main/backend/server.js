...
const db = new sqlite3.Database(dbSource, (err) => {
  if (err) {
    ...
    db.serialize(() => {
        ...
        db.run(`ALTER TABLE Complaints ADD COLUMN ai_detected_issue TEXT`, [], (err) => {});
        db.run(`ALTER TABLE Complaints ADD COLUMN confidence_score REAL`, [], (err) => {});
        db.run(`ALTER TABLE Complaints ADD COLUMN risk_score INTEGER`, [], (err) => {});
        db.run(`ALTER TABLE Complaints ADD COLUMN severity_level TEXT`, [], (err) => {});
        db.run(`ALTER TABLE Complaints ADD COLUMN duplicate_of INTEGER`, [], (err) => {});
...
app.post('/api/report', authenticateToken, upload.single('image'), async (req, res) => {
    ...
    try {
        ...
        if (req.file) {
            ...
            classifyData.append('image', fs.createReadStream(req.file.path));
            const classifyRes = await axios.post('http://localhost:8000/ai/classify-issue', classifyData, { headers: classifyData.getHeaders() });
            if (classifyRes.data) {
                aiDetectedIssue = classifyRes.data.detected_issue;
                confidenceScore = classifyRes.data.confidence;
            }

            // Check duplicate
            const dupData = new FormData();
            dupData.append('image', fs.createReadStream(req.file.path));
            dupData.append('latitude', lat);
            dupData.append('longitude', lng);
            const dupRes = await axios.post('http://localhost:8000/ai/check-duplicate', dupData, { headers: dupData.getHeaders() });
            if (dupRes.data && dupRes.data.is_duplicate) {
                isDuplicate = true;
                duplicateOf = dupRes.data.duplicate_complaint_id;
            }
        }

        // Check risk score
        const riskData = new FormData();
        riskData.append('issue_type', aiDetectedIssue || category);
        riskData.append('duplicate_count', isDuplicate ? 1 : 0);
        riskData.append('location_type', 'normal');
        const riskRes = await axios.post('http://localhost:8000/ai/risk-score', riskData, { headers: riskData.getHeaders() });
        if (riskRes.data) {
            riskScore = riskRes.data.risk_score;
            severityLevel = riskRes.data.severity_level;
        }
        ...
        const stmt = db.prepare(`INSERT INTO Complaints 
            (user_id, category, subcategory, description, image_url, lat, lng, ai_detected_issue, confidence_score, risk_score, severity_level) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
        
        stmt.run([userId, category, subcategory, description, imageUrl, lat, lng, aiDetectedIssue, confidenceScore, riskScore, severityLevel], function (err) {
            if (err) {
                return res.status(500).json({ error: err.message });
            }
            res.status(201).json({ 
        ...
        // Fallback to regular insert if AI fails
        ...
        stmt.run([userId, category, subcategory, description, imageUrl, lat, lng], function (err) {
            ...
            res.status(201).json({ id: this.lastID, message: 'Complaint created successfully (AI fallback)' });
        });
        stmt.finalize();
    }
});
